...
export interface ProgressSnapshot {
  ...
  xpTarget: number;
  xpProgress: number;
  /** Alias legado para xpProgress (compat CareerProgress.tsx) */
  xpProgressPct: number;
  xpRemaining: number;
  directCurrent: number;
  directTarget: number;
  directProgress: number;
  /** Alias legado para directProgress (compat CareerProgress.tsx) */
  directProgressPct: number;
  directRemaining: number;
  /** Nível seguinte a atingir (obj com metadados do pack) */
  nextLevel: {
    label: string;
    subtitle: string;
    requiredXp: number;
