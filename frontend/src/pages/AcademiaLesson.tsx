...
function buildRawMdUrl(mdPath?: string) {
  if (!mdPath) return "";
  return /\.(md|json)$/i.test(mdPath) ? `${REPO_RAW}/${mdPath}` : "";
}

function buildRepoUrl(mdPath?: string) {
  return mdPath ? `${REPO_BLOB}/${mdPath}` : "";
}
...
export default function AcademiaLesson() {
  ...
  const rawMdUrl = buildRawMdUrl(lesson.mdPath);
