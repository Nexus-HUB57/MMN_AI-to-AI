...
os domínios `commissions`, `marketplace` e `agent-runtime` passaram também a contar com
...
5. Replicar o padrão de `commissions` / `marketplace` / `agent-runtime` (`types.ts` + `repository.ts` + `service.ts`) nos demais domínios priorizados.
